...
| Widget colors (Premium) | `WidgetColorStyle` + `WidgetColorPickerSheet` (app group) | Skipping Tournament theme default on `next-*` / `countdown` / `calendar`; unifying all defaults to one black (content-based colors are canonical: favorite OLED black, ATP hardcourt navy, WTA berry, live near-black, order AO sky — all with the shared **neon-grid** texture, Jul 2026); re-adding a glowing horizon line to `NeonGridOverlay` (read as a green glitch/strikethrough across every widget — floor grid only) |
...
- **Widget typography:** `WidgetTheme.displayFont` (rounded heavy + `.courtifyScoreboardNumber` kerning) for ranks / countdowns / scores; `.courtifyMicroLabel` for DAYS/WINS unit labels — no stencil / compressed default design.
