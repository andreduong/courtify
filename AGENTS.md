...
| Widget colors (Premium) | `WidgetColorStyle` + `WidgetColorPickerSheet` (app group) | Skipping Tournament theme default on `next-*` / `countdown` / `calendar`; reverting favorite/standings/live/order to colorful gradient defaults (unified OLED black + neon-grid + `WidgetLEDAccent` is canonical, Jul 2026); brightening the `NeonGridOverlay` horizon (strikethrough-over-copy trap) |
